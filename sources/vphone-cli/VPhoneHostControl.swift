...
class VPhoneHostControl {
    ...
    return await withCheckedContinuation { continuation in
      ...
      let block: CompletionBlock = { imageObject in
        ...
        if let nsImage = imageObject as? NSImage {
          continuation.resume(
            returning: nsImage.cgImage(forProposedRect: nil, context: nil, hints: nil))
          return
        }
  ...
  private nonisolated static func handleClient(_ fd: Int32, controller: VPhoneHostControl?) {
        ...
        do {
          ...
          if let outputPath {
            let url = try await recorder.saveScreenshot(
              view: view, to: URL(fileURLWithPath: outputPath))
    ...
    case "key":
      ...
      let hidKey: (page: UInt32, usage: UInt32)? =
        switch name {
        case "home": (0x0C, 0x40)
        case "power": (0x0C, 0x30)
        case "volup": (0x0C, 0xE9)
        case "voldown": (0x0C, 0xEA)
        case "spotlight": (0x07, 0xE3)  // placeholder, combo handled below
        default: nil
        }
      ...
      Task { @MainActor in
        ...
        if name == "spotlight" {
          // Cmd+Space combo (VPhoneKeyHelper.sendSpotlight equivalent)
          ctl.sendHIDDown(page: 0x07, usage: 0xE3)  // Cmd down
          ctl.sendHIDPress(page: 0x07, usage: 0x2C)  // Space press
          ctl.sendHIDUp(page: 0x07, usage: 0xE3)  // Cmd up
        } else {
    ...
    case "app_launch":
      ...
      semaphore.wait()
      writeResponse(
        fd, ok: result.ok, path: result.path, error: result.error, image: result.imageBase64)
