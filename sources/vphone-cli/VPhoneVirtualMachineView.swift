...
class VPhoneVirtualMachineView: VZVirtualMachineView {
 ...
 private func pixelToLocal(pixelX: Double, pixelY: Double, screenWidth: Int, screenHeight: Int)
  -> NSPoint
 {
  let w = bounds.width
  let h = bounds.height
 ...
 func injectSwipe(
  ...
  let startLocal = pixelToLocal(
   pixelX: fromX, pixelY: fromY, screenWidth: screenWidth, screenHeight: screenHeight)
  let endLocal = pixelToLocal(
   pixelX: toX, pixelY: toY, screenWidth: screenWidth, screenHeight: screenHeight)
 ...
 /// Inject a touch-based swipe using proper UITouch events (not mouse events).
 /// Required for iOS edge gestures (CC, NC) which use UIScreenEdgePanGestureRecognizer.
 func injectTouchSwipe(
  fromX: Double, fromY: Double, toX: Double, toY: Double,
  screenWidth: Int, screenHeight: Int, durationMs: Int = 500
 ) {
  let startLocal = pixelToLocal(
   pixelX: fromX, pixelY: fromY,
   screenWidth: screenWidth, screenHeight: screenHeight)
  let endLocal = pixelToLocal(
   pixelX: toX, pixelY: toY,
   screenWidth: screenWidth, screenHeight: screenHeight)
  ...
  for i in 1..<steps {
   ...
   DispatchQueue.main.asyncAfter(deadline: .now() + stepInterval * Double(i)) { [weak self] in
    _ = self?.sendTouchEvent(phase: 1, localPoint: pt, timestamp: now + stepInterval * Double(i))
   }
  }
  DispatchQueue.main.asyncAfter(deadline: .now() + stepInterval * Double(steps)) { [weak self] in
   _ = self?.sendTouchEvent(
    phase: 2, localPoint: endLocal,
    timestamp: now + stepInterval * Double(steps))
  }
 }
